...
1. **Conflicts are classified correctly on this branch, but not on its siblings.** `ES-4`
   builds the storage engine with `.persistenceExceptionResolver(SQLStateResolver())`, so a
   Postgres `23505` becomes a `ConcurrencyException` and `ConcurrencyRetryScheduler` does
   retry it (5 attempts, 25ms·2ⁿ capped at 500 ms). `ES-1`/`ES-2`/`ES-3` lack that resolver
   entirely, so there the conflict surfaces as a generic `EventStoreException` and is never
   retried at all. Retrying only narrows the window — see (2) for what happens on exhaustion.
