...
`the-dashboard` can already be pointed at the archive (Data source -> "Prometheus Replay"),
but only one run at a time and only by typing that run's absolute window into the time
picker -- which makes comparing two runs an exercise in copying epoch timestamps out of
meta.json. This module generates a third dashboard, `bench-runs`, where picking a run from a
dropdown redraws every panel over an axis that always starts at the run's own t0.
...
The anchor sits AFTER the whole campaign, not at replay_run.py's 2026-01-01 origin, because
reaching forward in time needs a negative offset and Prometheus rejects those unless started
...
This is the full-fidelity view -- real scraped TSDB, all 53 live panels, the pg_stat_*/WAL/
HikariCP/outbox panels that no dump.json can carry. What it deliberately cannot do is overlay
two runs as separate lines in one panel: a single query carries a single offset. Overlaying
needs run_id stamped into the data itself, which is what bench-replay does (from dump.json,
~20 panels). Use `bench-runs` to look at one run in full, `bench-replay` to overlay several.
"""
import datetime
import glob
import os
...
def scan_runs(roots):
    ...
    Runs missing either are skipped: an aborted run has nothing to show, and a run archived
    from dump.json alone would fill all 53 panels with "No data" and no indication that the
...
def build_runs(found):
    ...
    dash = build._base(
        ...
        "merged into bench-replay-data, not a dump.json reconstruction. Pick a run from the "
        ...
        "For overlaying several runs in one panel, use the Bench Replay dashboard instead.",
        ANCHOR_ISO, end_iso,
