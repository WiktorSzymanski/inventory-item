...
def _var(name, label, query, datasource, multi=False, regex=""):
    ...
    # unified (`inventory-es` -> `inventory`, `postgres-es` -> `mongo`, `api-es-1` ->
...
def _const_var(name, label, value):
    ...
    # pins a container_name, the api included, so cadvisor reports plain `mongo` and `api`
...
def build_live():
    dash = _base(
        ...
        "with the PostgreSQL section re-pointed at MongoDB. "
        ...
        #   job    prometheus.yml scrapes jobs `inventory`, `mongo` and `cadvisor`;
        #          only `inventory` carries the application metrics.
        #   db     mongodb_dbstats_* also reports admin/config/local.
        #   dbc    the DB container is named `mongo`. Must NOT catch `mongodb-exporter` or
        #          `mongo-init`,
        ...
        [_var("job", "API job", "label_values(up, job)", DS, regex="/^inventory$/"),
         _var("db", "Database", "label_values(mongodb_dbstats_dataSize, database)", DS, regex="/^inventory$/"),
         _const_var("dbc", "DB container", "mongo"),
