...
data class SagaProcessorProperties(
    ...
    val replicas: Int = 1,
    // Width of the `saga-command` pool, which since the 2026-08-20 port from ES-4 is the ONLY
    // execution lane: first attempts, retries and the saga's terminal dispositions all run there
    // (see CommandGatewayConfig.RETRY_POOL_SIZE for why the retry lane executes nothing).
    //
