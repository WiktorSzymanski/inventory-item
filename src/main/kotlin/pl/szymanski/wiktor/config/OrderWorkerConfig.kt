 ...
 *    exactly what it was before the merge — which matters because `InventoryService.onOrderCreated`,
 *    the `@ApplicationModuleListener` that marks the publication complete, is `@Async` and this is
 *    the only `TaskExecutor` bean in the context, so it runs here.
 ...
 *    executor would back off Boot's `taskScheduler`, and `@Scheduled` work — the backup republisher
 *    in `PollingEventPublicationConfig` (30 s) and `OutboxMetrics` (5 s) — would migrate onto the
 *    order pool. That is the "a retry wait parked on the outbox scheduler" failure the retry
...
class OrderWorkerConfig {
     ...
     *
     * NOTE this pool runs more than the order tasks. Because an Executor bean exists in this
     * context, Boot's `applicationTaskExecutor` is never created, and Spring's @Async resolution
     * then finds exactly one TaskExecutor bean — this one. So `InventoryService.onOrderCreated`,
     * the @ApplicationModuleListener that marks the publication complete in its own REQUIRES_NEW
     * transaction, ALSO runs here: event DELIVERY competes with reservation EXECUTION for these
     * threads and their queue. Long-standing behaviour on every TO branch, called out because it is
     * invisible, and deliberately unchanged — the merged pool must be the only variable.
     */
