...
class CommandGatewayConfig {
    ...
    companion object {
         ...
         * So does a run that exports `COMMAND_POOL` above 112 without moving AXON_JDBC_POOL_SIZE
         * with it: the command term in that sum is the knob, and it is the term with the largest
         * coefficient. Watch the [POOLS] line, which recomputes the whole sum from what is
         * actually configured.
         *
         ...
         * the saga's terminal dispositions all run here. 112 = the parent's 82 + the 30 the retry
         * lane no longer executes on, which is what keeps the connection budget above identical to
         * the parent's.
         *
