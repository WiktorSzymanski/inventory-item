    ...
    fun write(outcome: OrderReserveOutcome) {
        ...
        try {
            ...
            val written = try {
                ...
                // Only the row that lost is stale — the rest of this order's rows still match what
                // the cache holds, because the rollback put the database back where they read it.
                // Evicting the whole order's worth would also be correct, and would understate the
                // hit rate on exactly the contended orders this branch exists to measure.
            ...
            // and the supertype of both catches above, which have already evicted by the time the
            // rethrow lands here. Timed and rethrown: the attempt still fails, it just stops being
            // invisible. Anything else (NotFound, a driver fault) is left unsampled — it is not the
            // biased mass, and giving it a bucket would only invite it to be read as write cost.
