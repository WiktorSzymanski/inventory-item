...
class OutboxMetrics(
     ...
     *
     * MEANINGLESS in scan mode. With `app.outbox-cursor.enabled=false` nothing writes the cursor,
     * so this climbs forever while delivery is perfectly healthy — it is the gap to a position no
     * one is keeping, not a backlog. `outbox.backlog` is the series to read in that arm.
     */
