...
class ReserveOrderItemsTransactionBoundaryTest {
    ...
    class TestConfig {
        ...
        /**
         * Empty, so every item is a miss and the read phase still issues its SELECT — which is the
         * call this test probes for a transaction. A pre-populated cache would skip it and the
         * probe would read null rather than false.
         */
        @Bean fun inventoryStateCache(): Cache<String, InventoryItem> = Caffeine.newBuilder().build()
        ...
        fun orderWriteCommandHandler(
            ...
            inventoryStateCache: Cache<String, InventoryItem>,
            meterRegistry: MeterRegistry,
        ) = OrderWriteCommandHandler(
            inventoryBatchWriter, orderRepository, applicationEventPublisher, inventoryStateCache, meterRegistry,
        )
        ...
        fun reserveOrderItemsCommandHandler(
            ...
            inventoryRepository, orderRepository, orderWriteCommandHandler, inventoryStateCache, clock, meterRegistry,
        )
    }
