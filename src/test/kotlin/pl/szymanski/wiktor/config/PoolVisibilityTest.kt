 ...
 * indistinguishable in a `jcmd Thread.print` — and that distinction is exactly what the one-pool
 * topology has to be checked against. With no gauges, a run that got faster and a run that merely
 * moved its backlog from one queue to the next look identical.
 */
class PoolVisibilityTest {
    ...
    fun `both pools publish active, queued and size`() {
        ...
        // this branch shipped before the port.
        ...
        // change thread-creation timing against the two-lane runs and break the topology-only
        // comparison.
